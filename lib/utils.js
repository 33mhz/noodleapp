...
var createClientMessage = function(client, options, callback) {
  ...
  userDb.isStarred(options.userId, options.recent.id, client, function(err, starred) {
    var userReferences = ['@' + options.user.username];
    var replyTo = null;

    options.userMentions.forEach(function(user) {
      if (userReferences.indexOf('@' + options.user.name) === -1) {
        userReferences.push('@' + options.user.name);
      }
    });

    userReferences.splice(userReferences.indexOf('@' + options.user.username), 1);

    if (userReferences.indexOf('@' + options.username) > -1) {
      userReferences.splice(userReferences.indexOf('@' + options.username), 1);
    }

    if (options.recent.you_starred || starred) {
      userDb.star(options.userId, options.recent.id, client);
      isStarred = true;
    } else {
      userDb.unstar(options.userId, options.recent.id, client);
    }

    userDb.isReposted(options.userId, options.recent.id, client, function(err, reposted) {
      var isRepost = false;

      if (options.recent.you_reposted || reposted) {
        userDb.repost(options.userId, options.recent.id, client);
        isRepost = true;
      } else {
        userDb.unrepost(options.userId, options.recent.id, client);
      }
      ...
      if (recent.text) {
        ...
        recent.annotations.forEach(function(annotation) {
          if (annotation.type === 'net.xtendr.mood' && moodArray.indexOf(annotation.value.mood) > -1) {
            ...
            if (annotation.value.type === 'photo' && recent.text.indexOf(annotation.value.url) === -1) {
