  ...
  process.nextTick(function() {
    ...
    if (recentMessages.length > 0) {
      for (var i = 0; i < recentMessages.length; i ++) {
        ...
        if (typeof recent.is_deleted === 'undefined' && typeof recent.content.text !== 'undefined') {
          messageCount ++;
          ...
          if (recent.repost_of)
            recent.raw = recent.repost_of.raw;
          
          if (recent.raw) {
            recent.raw.forEach(function(annotation) {
              if (raw.type === 'net.xtendr.mood' && moodArray.indexOf(annotation.value.mood) > -1) {
