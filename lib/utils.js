...
var createClientMessage = function(options, callback) {
  process.nextTick(function() {
    ...
    if (typeof options.user !== 'undefined' && options.user.id === options.userId) {
      isSelf = true;
    }
    
    if (typeof options.recent.counts === 'undefined') {
        options.recent.counts = {replies:0,bookmarks:0,reposts:0};
    }
    ...
    var messageData = {
      ...
      user: options.user.content.avatar_image.url,
      ...
      appSourceUrl: options.recent.source.url,
  ...
  process.nextTick(function() {
    ...
    if (recentMessages.length > 0) {
      for (var i = 0; i < recentMessages.length; i ++) {
        ...
        if (typeof recent.is_deleted !== 'undefined') {
        ...
        } else if (typeof recent.content !== 'undefined' && typeof recent.content.entities !== 'undefined') {
          messageCount ++;

          if (recent.repost_of) {
            var repost = recent;
            recent = recent.repost_of;
            recent.repostDetails = { id: repost.id, user_id: repost.user.id, username: repost.user.username, avatar: repost.user.content.avatar_image.url };
          }
          ...
          if (recent.raw) {
            if (recent.raw['io.pnut.core.oembed']) {
              recent.raw['io.pnut.core.oembed'].forEach(function(annotation) {
                if (annotation.type === 'photo' &&
                  recent.content.text.indexOf(annotation.url) === -1) {
                  var linkUrl = annotation.embeddable_url || annotation.url;
                  var photo = '<div class="image-wrapper"><a href="' + linkUrl +
                    '" target="_blank"><img src="' + annotation.url + '" alt="" title=""></div>' +
            ...
            if (recent.raw['io.pnut.core.crosspost']) {
              recent.raw['io.pnut.core.crosspost'].forEach(function(annotation) {
                crosspost = annotation.canonical_url;
              });
            }
            if (recent.raw['nl.chimpnut.blog.post']) {
              recent.raw['nl.chimpnut.blog.post'].forEach(function(annotation) {
    ...
    recentList.forEach(function(recent) {
      ...
      try {
          ...
          if (recent.action === 'reply') {
            message = 'Replied to <a href="/#/post/' + messageItem.id + '/' + username + '">your post</a>:<p>' + messageItem.content.html + '</p>';
            ...
            message = 'Bookmarked <a href="/#/post/' + messageItem.id + '/' + username + '">your post</a>:<p>' + messageItem.content.html + '</p>';
            icon = '<span class="action-icon star"></span>';
          } else if (recent.action === 'repost') {
            message = 'Reposted <a href="/#/post/' + messageItem.id + '/' + username + '">your post</a>:<p>' + messageItem.content.html + '</p>';
          ...
          newMessages.unshift(options);

          if (paginated) {
            newMessages = newMessages.sort(function(a, b) {
              return parseInt(b.pagination_id, 10) - parseInt(a.pagination_id, 10);
            });
          }
      } catch (err) {
          console.log(recent);
