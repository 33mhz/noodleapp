...
var MOOD_ARRAY = ['happy', 'sad', 'tired', 'angry', 'calm', 'omg'];
...
var processLink = function(idx, entities, client, options, callback) {
  ...
  webremix.generate(link, client, function(errMsg, message) {
    ...
    if (idx === options.recent.entities.links.length - 1) {
      ...
      processLink(idx ++, entities, client, options, callback);
    }
  });
};

var setAnnotations = function(req, recent, recentMessages, messageCount, metadata, newMessages, paginated) {
  var userSession = exports.getUser(req);
  ...
  var entities = [];
  var recentEntities;
  var recentAnnotations = recent.annotations;

  // Annotations
  if (recentAnnotations) {
    recentAnnotations.forEach(function(annotation) {
      if (annotation.type === 'net.xtendr.mood' && MOOD_ARRAY.indexOf(annotation.value.mood) > -1) {
        ...
        if (annotation.value.type === 'photo' && text.indexOf(annotation.value.url) === -1) {
  ...
  if (userMentions) {
    ...
    userMentions.forEach(function(mention) {
      var name = text.substr(mention.pos, mention.len);
      entities.push({
        pos: mention.pos,
        len: mention.len,
  ...
  var hashtags = recent.entities.hashtags;

  // Hashtags
  if (hashtags) {
    hashtags.forEach(function(hashtag) {
      var name = text.substr(hashtag.pos, hashtag.len);
      entities.push({
        pos: hashtag.pos,
        len: hashtag.len,
  ...
  process.nextTick(function() {
    ...
    if (recentMessages.length > 0) {
      recentMessages.forEach(function(recent) {
        ...
        if (recent.text) {
          var annotationObj = setAnnotations(req, recent, recentMessages, messageCount, metadata, newMessages, paginated);

          if (recent.entities.links.length > 0) {
            processLink(0, annotationObj.entities, client, annotationObj.options, callback);
          } else {
            continueRendering(annotationObj.entities, client, annotationObj.options, callback);
          }
        }
      });
    } else {
  ...
  if (recentList.length > 0) {
    recentList.forEach(function(recent) {
      ...
      if (recent.action === 'reply') {
        message = 'Replied to <a href="/#/post/' + messageItem.id + '/' + req.session.username + '">your post</a>';
      } else if (recent.action === 'follow') {
        message = 'Followed you';
      } else if (recent.action === 'star') {
        message = 'Starred <a href="/#/post/' + messageItem.id + '/' + req.session.username + '">your post</a>';
      } else if (recent.action === 'repost') {
        message = 'Reposted <a href="/#/post/' + messageItem.id + '/' + req.session.username + '">your post</a>';
      }
