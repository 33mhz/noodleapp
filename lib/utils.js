 ...
 * Requires: web request, callback
 * Returns: the image url if successful, otherwise an error
 */
exports.uploadImage = function(req) {
  ...
  request.post('http://api.imgur.com/2/upload.json?' + qs.stringify(params), function(err, resp, body) {
    if (err) {
      callback(err);
    } else {
      console.log('received uploaded image ******* ', JSON.parse(body).upload.links.imgur_page);
    }
  });
};
  ...
  if (recentMessages.length > 0) {
    recentMessages.forEach(function(recent) {
      ...
      if (recent.text) {
        ...
        webremix.generate(recent.text, client, function(errMsg, message) {
          if (!errMsg) {
            ...
            recent.annotations.forEach(function(annotation) {
              if (annotation.type === 'net.xtendr.mood') {
                ...
                if (annotation.value.type === 'photo') {
                  photo = annotation.value.url;
                  photoEmbedUrl = annotation.value.embeddable_url;
                }
              }
            });
