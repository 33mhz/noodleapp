...
// markdown can have markdown [test](url) links, or bare URLs.  Anything else will be ignored and remain text.  Only link entities are set; that's all that's needed to post to the pnut.io API.
exports.parse = function(markdown) {
  ...
  return {
    text: text,
    // commented out for now, as the API currently ignores them but doesn't parse links itself if present
    // entities: {
      // links: links
    // }
  };
...
exports.stringify = function(adnObject) {
  ...
  for(var i = links.length - 1; i >= 0; i--) {
    ...
    if(link.text !== link.link) {
      ...
      markdown = markdown.substring(0, link.pos) + '[' + link.text + '](' + link.link + ')' + remaining;
    }
  }
