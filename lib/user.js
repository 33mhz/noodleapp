    ...
    if (err) {
      ...
      appnet.getUser(req, user.username, function(err, user) {
        if(err) {
          ...
          if (typeof userItems.name !== 'undefined') {
            userItems.name = user.name;
          } else {
            userItems.name = '@'+userItems.username;
          }
          ...
          if (user.content) {
            try {
              userItems.content = markdownToEntities.stringify(user.content);
            } catch(e) {
  ...
  client.hmset('settings:' + user.id, localSettings, function(err, resp) {
    if (err) {
      ...
      var userInfo = {
        ...
        content: markdownToEntities.parse(settings.content)
      };
