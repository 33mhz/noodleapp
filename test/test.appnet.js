  ...
  describe('.userPosts',  function() {
    it('returns posts by a user', function(done) {
      var scope = nock(APPNET_URL)
        .get('/stream/0/users/1/posts?since_id=1&before_id=1&include_deleted=0')
        .reply(200, POST_JSON);
    ...
    it('returns the global feed', function(done) {
      var scope = nock(APPNET_URL)
        .get('/stream/0/posts/stream/global?since_id=1&before_id=1&include_deleted=0')
        .reply(200, POST_JSON);
    ...
    it('returns a paginated global feed', function(done) {
      ...
      var scope = nock(APPNET_URL)
        .get('/stream/0/posts/stream/global?since_id=&before_id=1&include_deleted=0')
        .reply(200, POST_JSON);
    ...
    it('returns paginated user posts', function(done) {
      ...
      var scope = nock(APPNET_URL)
        .get('/stream/0/users/1/posts?since_id=&before_id=1&include_deleted=0')
        .reply(200, POST_JSON);
      appnet.paginatedFeed(req, client, function(err, resp) {
        resp.data[0].id.should.equal('1')
        done();
    ...
    it('returns a created post', function(done) {
      ...
      var scope = nock(APPNET_URL)
        .post('/stream/0/posts?access_token=1', qs.stringify(params))
        .reply(200, POST_JSON);
